...
Reproducible output is enforced as a build property, not inferred from a fixed uImage header. Linux 3.4 receives a locked build timestamp, user, host and version; BusyBox and every SquashFS inode use the Source Lock epoch in UTC. CI then performs a second clean build with the same provisioning inputs and rejects any byte difference before sealing `reproducibility-policy.json` into the checksummed Firmware Bundle. Public runs still use fresh random credentials between separate runs, so reproducibility is proved within each run using the same protected inputs.
